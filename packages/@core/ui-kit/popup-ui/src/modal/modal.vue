...
import {
  ...
  usePriorityValues,
  useSimpleLocale,
} from '@repo-core/composables';
import { Expand, Shrink } from '@repo-core/icons';
import {
  Dialog,
  DialogContent,
  DialogDescription,
  ...
  repoLoading,
  VisuallyHidden,
} from '@repo-core/shadcn-ui';
import { ELEMENT_ID_MAIN_CONTENT } from '@repo-core/shared/constants';
import { globalShareState } from '@repo-core/shared/global-state';
...
<template>
  ...
  >
    ...
    >
        ...
        <DialogTitle v-if="title" class="text-left">
          <slot name="title">
            ...
            <slot v-if="titleTooltip" name="titleTooltip">
              <repoHelpTooltip trigger-class="pb-1">
                {{ titleTooltip }}
              </repoHelpTooltip>
            </slot>
          </slot>
      ...
      <repoLoading v-if="showLoading || submitting" spinning />
      <repoIconButton
        v-if="fullscreenButton"
        class="hover:bg-accent hover:text-accent-foreground text-foreground/80 flex-center absolute top-3 right-10 hidden size-6 rounded-full px-1 text-lg opacity-70 transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none sm:block"
      ...
      >
        ...
        <slot name="footer">
          <component
            :is="components.DefaultButton || repoButton"
          ...
          <component
            :is="components.PrimaryButton || repoButton"
