...
import type { repoButtonGroupProps, ValueType } from './button';

import { computed, ref, watch } from 'vue';

import { Circle, CircleCheckBig, LoaderCircle } from '@repo-core/icons';
import { cn, isFunction } from '@repo-core/shared/utils';

import { objectOmit } from '@vueuse/core';

import { repoRenderContent } from '../render-content';
import repoButtonGroup from './button-group.vue';
import Button from './button.vue';

const props = withDefaults(defineProps<repoButtonGroupProps>(), {
  gap: 0,
  multiple: false,
  showIcon: true,
  size: 'middle',
  ...
  >
    ...
    >
      ...
      <slot name="option" :label="btn.label" :value="btn.value" :data="btn">
        <repoRenderContent :content="btn.label" />
      </slot>
    </Button>
  </repoButtonGroup>
</template>
<style lang="scss" scoped>
.repo-check-button-group {
  display: flex;
  flex-wrap: wrap;
