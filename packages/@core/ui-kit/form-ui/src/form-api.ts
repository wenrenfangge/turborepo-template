...
import type { FormActions, FormSchema, repoFormProps } from './types';

import { isRef, toRaw } from 'vue';

import { Store } from '@repo-core/shared/store';
import {
  bindMethods,
  createMerge,
  formatDate,
...
function getDefaultState(): repoFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  // private api: Pick<repoFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | repoFormProps = null;
  stateHandler: StateHandler;

  public store: Store<repoFormProps>;
  ...
  private prevState: null | repoFormProps = null;

  constructor(options: repoFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<repoFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  setState(
    stateOrFn:
      | ((prev: repoFormProps) => Partial<repoFormProps>)
      | Partial<repoFormProps>,
  ) {
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<repoForm /> is not mounted');
    }
    return this.form;
  }
