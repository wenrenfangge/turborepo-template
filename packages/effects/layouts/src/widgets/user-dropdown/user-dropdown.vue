...
import type { AnyFunction } from '@repo/types';

import { computed, useTemplateRef, watch } from 'vue';

import { useHoverToggle } from '@repo/hooks';
import { LockKeyhole, LogOut } from '@repo/icons';
import { $t } from '@repo/locales';
import { preferences, usePreferences } from '@repo/preferences';
import { useAccessStore } from '@repo/stores';
import { isWindowsOs } from '@repo/utils';

import { userepoModal } from '@repo-core/popup-ui';
import {
  Badge,
  DropdownMenu,
  DropdownMenuContent,
...
const [LockModal, lockModalApi] = userepoModal({
  connectedComponent: LockScreenModal,
});
const [LogoutModal, logoutModalApi] = userepoModal({
  onConfirm() {
    handleSubmitLogout();
  },
});
    ...
    <DropdownMenuTrigger ref="refTrigger" :disabled="props.trigger === 'hover'">
      <div class="hover:bg-accent mr-2 ml-1 cursor-pointer rounded-full p-1.5">
        <div class="hover:text-accent-foreground flex-center">
          <repoAvatar :alt="text" :src="avatar" class="size-8" dot />
        </div>
      </div>
    </DropdownMenuTrigger>
    <DropdownMenuContent class="mr-2 min-w-[240px] p-0 pb-1">
      <div ref="refContent">
        ...
        <DropdownMenuItem
          ...
          @click="menu.handler"
        >
          <repoIcon :icon="menu.icon" class="mr-2 size-4" />
          {{ menu.text }}
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem
