...
import { repoRenderContent } from '@repo-core/shadcn-ui';
...
const renderLink = (href: string, text: string) =>
  h(
    'a',
    { href, target: '_blank', class: 'repo-link' },
    { default: () => text },
  );
...
} = __repo_ADMIN_METADATA__ || {};

const repoDescriptionItems: DescriptionItem[] = [
  {
    content: version,
    title: '版本号',
  },
  ...
  {
    content: renderLink(repo_DOC_URL, '点击查看'),
    title: '文档地址',
  },
  {
    content: renderLink(repo_PREVIEW_URL, '点击查看'),
    title: '预览地址',
  },
  {
    content: renderLink(repo_GITHUB_URL, '点击查看'),
    title: 'Github',
  },
  {
    content: h('div', [
  ...
  <Page :title="title">
    <template #description>
      <p class="text-foreground mt-3 text-sm leading-6">
        <a :href="repo_GITHUB_URL" class="repo-link" target="_blank">
          {{ name }}
        </a>
        {{ description }}
      </p>
    ...
    <div class="card-box p-5">
      ...
      <div class="mt-4">
        <dl class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <template v-for="item in repoDescriptionItems" :key="item.title">
            <div class="border-border border-t px-4 py-6 sm:col-span-1 sm:px-0">
              ...
              <dd class="text-foreground mt-1 text-sm leading-6 sm:mt-2">
                <repoRenderContent :content="item.content" />
              </dd>
            </div>
          </template>
        </dl>
